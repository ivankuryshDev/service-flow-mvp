...
export function BookingSection({ service, provider, slotGroups }: BookingSectionProps) {
  ...
  const selectedSlot = slotGroups
    .flatMap((g) => g.slots)
    .find((s) => s.id === selectedSlotId) ?? null;
  ...
  return (
    <div className="grid gap-8 lg:grid-cols-[1fr_360px]">
      ...
      <div className="flex flex-col gap-8">
          ...
          {slotGroups.length === 0 ? (
            ...
            <div className="mt-4 flex flex-col gap-5">
              {slotGroups.map((group) => (
                <div key={group.date}>
                  <p className="mb-2.5 text-sm font-semibold text-ink-700">{group.dateLabel}</p>
                  <div className="flex flex-wrap gap-2" role="group" aria-label={`Slots for ${group.dateLabel}`}>
        ...
        {/* Customer form */}
        <div>
          <p className="text-xs font-medium uppercase tracking-wide text-ink-400">Your details</p>
          ...
          >
            ...
            <button
              ...
              className="mt-1 inline-flex w-full items-center justify-center rounded-pill bg-yellow-400 px-6 py-3.5 text-base font-semibold text-ink-900 shadow-sm transition-all hover:bg-yellow-500 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-yellow-500 focus-visible:ring-offset-2 disabled:pointer-events-none disabled:opacity-50"
            >
              {selectedSlotId ? 'Confirm booking' : 'Select a time to continue'}
            </button>
            {!selectedSlotId && (
              <p className="text-center text-xs text-ink-400">Choose an available time slot above first.</p>
            )}
          </form>
        </div>
      </div>
        ...
        <div className="sticky top-6">
          <div className="overflow-hidden rounded-xl border border-border-soft bg-white shadow-sm">
            ...
            <div className="bg-ink-900 px-6 py-4">
              <p className="text-xs font-medium uppercase tracking-wide text-yellow-400">Your booking</p>
            ...
            <div className="divide-y divide-border-soft px-6">
              ...
              <div className="flex items-center justify-between py-3.5">
                ...
                <span className="text-sm font-medium text-ink-800">
                  {selectedGroup?.dateLabel ?? <span className="text-ink-300">Select a time</span>}
                </span>
              </div>
              <div className="flex items-center justify-between py-3.5">
                <span className="text-sm text-ink-500">Time</span>
                <span className={cn('text-sm font-medium', selectedSlot ? 'font-mono text-ink-800' : 'text-ink-300')}>
                  {selectedSlot ? `${selectedSlot.timeLabel} – ${selectedSlot.endTimeLabel}` : 'Select a time'}
                </span>
              </div>
            </div>
            ...
            <div className="bg-cream-50 px-6 py-4">
              <div className="flex items-end justify-between">
                ...
                <p className="font-display text-2xl font-bold text-ink-900">€{service.priceFrom}</p>
              </div>
            </div>

            {/* Reassurance */}
            <div className="px-6 py-4">
              <p className="text-center text-xs text-ink-400">Confirmed instantly · free to change up to 24h before</p>
            </div>
          </div>
        </div>
      </div>
